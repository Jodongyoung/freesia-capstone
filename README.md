...
<p align="center">
  <img src="[https://github.com/user-attachments/assets/cf501253-3825-4f1f-9398-089d450dff3f](https://github.com/user-attachments/assets/cf501253-3825-4f1f-9398-089d450dff3f)" width="32%">
  <img src="[https://github.com/user-attachments/assets/88ea404a-d14d-449b-b7af-9a0eef2750bb](https://github.com/user-attachments/assets/88ea404a-d14d-449b-b7af-9a0eef2750bb)" width="32%">
  <img src="[https://github.com/user-attachments/assets/5bbee359-6088-48e5-9c8d-42592cc4209b](https://github.com/user-attachments/assets/5bbee359-6088-48e5-9c8d-42592cc4209b)" width="32%">
</p>
...
│   │   ├── chat/           # 채팅 로직
│   │   ├── diary/          # 일기 및 감정 처리, N:M 매핑
│   │   ├── member/         # 회원 및 JWT 인증
│   │   └── recommendation/ # 콘텐츠 수집, Fallback 시드 풀 및 추천 스케줄러
│   └── Dockerfile
...
<img width="462" height="971" alt="erd_diagram" src="[https://github.com/user-attachments/assets/327eaa81-8b86-4f46-a32c-c7574314c563](https://github.com/user-attachments/assets/327eaa81-8b86-4f46-a32c-c7574314c563)" />
<br> * MySQL Workbench를 활용하여 설계한 Freesia 서비스의 데이터베이스 ERD (일기-추천 N:M 매핑 테이블 포함)

<br/>

## ✨ 핵심 기능 (Key Features)

1. **RAG 기반 능동형 공감 챗봇:** 자유 채팅 중 사용자의 과거 일기를 벡터 검색하여 시스템 프롬프트에 동적 주입, 과거 기억을 기반으로 먼저 안부를 묻는 공감형 대화 구현.
2. **인터랙티브 감정 캘린더 및 통계 대시보드:** 작성된 일기의 감정 점수 및 감정 분포를 Canvas 막대그래프와 월별 캘린더로 시각화하여 감정 변화 추이를 한눈에 제공.
3. **5종 맞춤 힐링 콘텐츠 큐레이션 (음악·영화·도서·취미·활동):** 분석된 감정에 어울리는 다양한 카테고리의 콘텐츠를 추천하며, 실시간 재추천(`[🔄 다른 콘텐츠 추천받기]`) 제공.
4. **일기별 추천 콘텐츠 영구 결속 (데이터 불변성):** 일기 작성 시 추천된 콘텐츠를 N:M 연관관계로 영구 보존하여, 캘린더에서 과거 일기 조회 시 작성 당시의 추천 콘텐츠가 그대로 유지.
5. **사용자 피드백 루프 (👍 찜 & 👎 다시 보지 않기):** 각 카드별 찜 토글 및 비추천 피드백을 `localStorage`와 백엔드 API에 영구 보존하여 개인화 추천 강화.

<br/>

## 🔥 핵심 트러블 슈팅 (Troubleshooting)

이 프로젝트는 단순 API 연동을 넘어 개발 과정에서 마주한 **구조적 한계, 외부 API 장애, 데이터 불일치 버그를 논리적으로 분석하고 해결하는 과정**에 집중했습니다.
...
- **해결:** 클라이언트 전송 시간을 무조건 신뢰하는 대신, Spring Boot 엔티티 생명주기 콜백(`@PrePersist`)을 활용하여 서버 단에서 KST(`Asia/Seoul`) 시간을 강제 오버라이트하도록 설계했습니다.

### 3. [Data] 추천 콘텐츠 시드 중복 및 무한 반복 현상
- **문제:** 추천 API 호출 시 `System.nanoTime()` 기반 시드가 루프 내에서 동일하게 적용되어 같은 콘텐츠만 편향 노출되는 문제 발생.
- **해결:** 매 추출마다 새로운 인스턴스로 무작위 셔플을 보장하고, 사용자의 '싫어요(Feedback)' 데이터를 스트림 필터로 분리하는 개인화 로직을 완성했습니다.

### 4. [Frontend] 긴 컨텐츠로 인한 UI 크래시(White Screen) 및 모달 이탈
- **문제:** 일기 본문과 5종 추천 카드가 길어질 경우 뷰포트(Viewport)를 벗어나거나 닫기 버튼이 화면 밖으로 밀려 클릭되지 않는 문제 발생.
...
- **문제:** YouTube Data API v3의 일일 무료 쿼터(10,000pt)가 빠르게 소진되어 `HTTP 429 Too Many Requests`가 발생, 서비스 전체 추천 기능이 마비되는 외부 API 종속성 장애 발생.
- **해결:** 장애 허용(Fault-Tolerance) 아키텍처를 도입하여, API Quota 초과 시에도 시스템 중단 없이 즉시 가동되는 **감정별 고품질 시드 데이터 풀(Seed Pool 48건)** 자동 적재 파이프라인을 구축했습니다. 또한 Video ID 만료 문제를 방지하기 위해 공식 음원 검색 딥링크(`results?search_query=`) 체계로 전환했습니다.
...
- **해결:** 
  1. DB에 `diary_recommendations` 다대다(N:M) 매핑 테이블을 신설하여 일기 생성 시 추천 콘텐츠를 영구 바인딩했습니다.
  2. 프론트엔드에서 모달 오픈 시 매번 랜덤 API를 부르던 구조를 제거하고, 일기 객체에 묶인 추천 데이터를 공유하는 **단일 원천(Single Source of Truth)** 구조로 동기화했습니다.
...
```bash
# 1. Repository 클론
$ git clone https://github.com/Jodongyoung/freesia-capstone.git
$ cd freesia-capstone
